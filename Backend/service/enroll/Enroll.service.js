...
async function handleGST(branchName, state, fees) {
    ...
    console.log(`state.toLowerCase() == "gujarat"`, state.toLowerCase() == "gujarat");
    
    if (state.toLowerCase() == "gujarat") {
        return {
            cGst: fees * matchedBranch.cGst / 100,
            sGst: fees * matchedBranch.sGst / 100,
            iGst: 0,
        }
    } else {
        return {
            iGst: fees * matchedBranch.iGst / 100,
            cGst: 0,
            sGst: 0,
        }
    }
...
exports.createEnrollDetail = async (enroll, isAdmin, isBranch) => {
    ...
    try {
        ...
        enroll.sGst = gstValues.sGst;
        enroll.iGst = gstValues.iGst;
...
exports.settleEnroll = async (enroll, isCN, isBranch) => {
    try {
        ...
        // Set the values to enroll
        findSettleEnrollStudent.cGst = gstValues.cGst;
        findSettleEnrollStudent.sGst = gstValues.sGst;
        findSettleEnrollStudent.iGst = gstValues.iGst;
...
exports.editEnrollDetail = async (data, token, isAdmin, isBranch) => {
    ...
    try {
        ...
        // Set the values to enroll
        data.cGst = gstValues.cGst;
        data.sGst = gstValues.sGst;
        data.iGst = gstValues.iGst;
        // Save the updated document
        const newPaymentSlip = {}
        ...
        if (body?.payAmount == true || body?.payAmount == 'true') {
            ...
            const modifiedPayments = payments.map(item => {
                if (item && item.payInstallmentFees) {
                    ...
                    sGst += item.sGst; // Increment totalPayAmount
                    cGst += item.cGst; // Increment totalPayAmount
                    return {
                        ...item._doc,
            ...
            const cGst = validResults.reduce((total, item) => {
                // Check if item is defined before accessing its properties
                if (item && item.payAmount) {
            ...
            const sGst = validResults.reduce((total, item) => {
                // Check if item is defined before accessing its properties
                if (item && item.payAmount) {
